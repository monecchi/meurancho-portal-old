...
const CardapioItems = (props) => {
  ...
  // Fetch all categories with custom useFetchMutate() swr hook
   ...
   * @param {number} index
   */
  const handleItemUpdate = (event, index) => {
  ...
  //
  // Handle quick edit product price and status
  //
  const handleQuickProductUpdate = useCallback((event, index) => {

    const target = event.target;
    const value = target.value;
    const name = target.name;

    const quickUpdateProduct = data.categories.products?.map((product, productIndex) => {
    ...
    mutate(quickUpdateProduct, false)
    mutate('/api/category/list', { ...data.categories.products }, false)

    // send a request to the API to update the source
    api.put(`/api/product/update/${quickUpdateProduct._id}`, quickUpdateProduct);

    mutateGlobal(`/api/product/item/${quickUpdateProduct._id}`, { quickUpdateProduct })
  }, [data, mutate]);


  /**
   * Handle displey/hide ActionBar
   * @param {boolean} value
   */
  ...
  //
  // Handle multiple price fields change
  //
  ...
  //
  // Handle quick update product status with swr mutate callback function
  //
  ...
  //
  // Handle array of items for component's initial state
  //
  const handleItemsForState = useCallback(() => {
    //const productItems = [];

    setInitialState({
      items: data && data.categories.map((category, index) => {
        return {
          ...category.products,
        }
      })
    ...
    // setInitialState({
    //   items: data && data.categories.map((item, index) => {
    //     return {
    //       ...item.products,
    //     }
    //   })
